...
This repo is a generated Next.js SaaS starter. Your job is to turn it into a specific product without breaking the working Stripe, auth, database, and deployment wiring that already exists.
...
1. Landing page first:
   ...
   - keep only product-relevant CTAs such as sign in, sign up, or checkout where appropriate
...
- Example palette directions:
  - minimal monochrome plus one signal color
...
- Do not invent a product narrative just because the starter is generic.
...
- Treat typography, color, motion, spacing, and composition as one system instead of isolated tweaks.
- Prefer a distinctive display font paired with a refined body font, and avoid generic default-looking UI choices.
- The build environment may not allow fetching Google Fonts, so local/system fonts are usually safer for implementation and verification.
- Use backgrounds, contrast, layering, and layout deliberately so the product does not look like a stock AI SaaS starter.
- Avoid cookie-cutter patterns such as generic copy, predictable layouts, timid color palettes, and overused AI aesthetics.
- Match implementation complexity to the visual direction: bold interfaces can be expressive, while restrained interfaces should feel precise and considered.
